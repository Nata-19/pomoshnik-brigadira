class BrigadeAssistant {
  ...
  _guideSteps() {
    return [
      { target: null, text: 'Это демо «Агроучёт работ». Покажу весь путь: как внести работу, проверить записи и получить готовые итоги.' },
      { target: '#i2-date', text: 'Сначала проверь дату. Все новые записи, явка и количество людей сохраняются именно на выбранный день.' },
      { target: '#estate-sel', text: 'Выбери культуру. У каждой культуры свой список кварталов, клеток и записей. Кнопка «+ ещё культура» добавляет новую.' },
      { target: '.chips-row', text: 'В блоке «Контекст» выбери квартал, клетку и вид работ. Ручные и механизированные работы находятся в разных списках.' },
      { target: '.add-inline', text: 'Если нужной работы нет, впиши её название, укажи «Ручной» или «Механизированный» и нажми «+ вид работ».' },
      { target: '.mode-row', text: 'В «Как считать» выбери единицу: ряды с растениями, только ряды, часы, гектары или километры. Поле ввода ниже изменится под выбранный режим.' },
      { target: null, text: 'Для режима «Гектары» отметь все клетки, где работал трактор, и впиши обработанные ряды. Площадь в гектарах приложение рассчитает по инвентарю. Для «Часов» и «Километров» введи готовое число.' },
      { target: '.roster-toggle', text: 'Открой «Выбрать рабочих из бригады» и отметь галочками всех, кто вышел сегодня. Новую фамилию можно добавить внизу списка.' },
      { target: '.chips', text: 'В маленьком поле «чел.» рядом с фамилией укажи общее количество людей за этой плашкой. Если это один человек — поставь 1.' },
      { target: '.chips', text: 'Теперь нажми на фамилию: синяя плашка означает, что работа будет записана выбранному сотруднику или группе.' },
      { target: null, text: 'Поле «Чел. на этот вид / квартал» нужно, если люди с одной плашки разделились. Например: всего 10 человек, из них 6 на обрезке и 4 на подвязке. Для обычной записи его можно не заполнять.' },
      { target: '#i2-rows', text: 'Введи результат работы. Ряды можно писать диапазоном и через запятую: «1-5, 9, 11». В других режимах здесь будут поля для часов или километров.' },
      { target: '#i2-rows-warn', text: 'Если указанного ряда нет в выбранной клетке, приложение сразу покажет предупреждение. Проверь квартал, клетку и номер ряда перед сохранением.' },
      { target: '#i2-add-btn', text: 'Нажми «Добавить». Без интернета запись тоже сохранится на телефоне и отправится на сервер, когда появится сеть.' },
      { target: '#i2-entries', text: 'В «Записи за дату» сразу появится отдельная карточка: кто работал, что делал, где и сколько. Ошибочную карточку можно удалить здесь.' },
      { target: '#i2-totals', text: 'В «Всего за день» приложение объединяет записи и показывает итог по ручным и механизированным работам.' },
      { target: '.accounting-export-actions', text: 'Под итогом находятся результаты для бухгалтерии: таблицей можно поделиться или скопировать её текст для отправки.' },
      { target: '#tab-logs', text: '«Журнал» показывает подробности за выбранную дату: работника, вид работы, квартал, клетку и номера рядов. Здесь также можно удалить ошибочную запись.' },
      { target: '#tab-report', text: 'В «Отчёте за период» укажи даты «От» и «До», затем нажми «Получить отчёт». Внизу появятся общие итоги и те же варианты выгрузки для бухгалтерии.' },
      { target: '#tab-reconcile', text: 'В «Сверке» выбери квартал, при необходимости клетку, и вид работ. Результат покажет, сколько сделано, что осталось и можно ли закрыть клетку.' },
      { target: '#tab-disputed', text: 'Если один ряд ввели повторно, приложение предложит переписать его, поделить между людьми или отложить. Отложенные конфликты разбираются во вкладке «Спорные».' },
      { target: '#tab-perf', text: '«Выполнение» — результат для агронома: сколько гектаров сделано сегодня, всего и сколько осталось. Отчёт можно отфильтровать и скопировать для отправки.' },
      { target: '.demo-reset-btn', text: 'Кнопка «Начать сначала» удаляет данные текущей демо-сессии. Используй её только если хочешь полностью очистить пример.' },
      { target: '#help-guide-btn', text: 'Готово. Если понадобится подсказка, нажми «❓ Как пользоваться» — гайд снова откроется с первого шага.' },
    ];
  }
...
previousGuideStep() {
    this._guideStep = Math.max(0, (this._guideStep || 0) - 1);
    try { localStorage.setItem('demo_guide_step', String(this._guideStep)); } catch (e) {}
    this._renderGuide();
  }
    ...
    el.innerHTML = `
      <div class="demo-guide-inner">
        ...
        <div class="demo-guide-buttons">
          ${isFirst ? '' : '<button class="demo-guide-back" onclick="app.previousGuideStep()">← Назад</button>'}
  ...
  _positionGuide(el, targetSelector) {
    ...
    const initialRect = tgt.getBoundingClientRect();
    if (initialRect.bottom < MARGIN || initialRect.top > window.innerHeight - MARGIN) {
